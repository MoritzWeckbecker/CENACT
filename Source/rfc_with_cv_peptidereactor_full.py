...
datasets_path = os.path.join('.', 'CMANGOES-2.0', 'Data_peptidereactor_full', 'Encodings')
...
for level in [2]:
    ...
    for alphabet_mode in ['data_driven']:
        for data_idx in range(len(datasets_list)):
            ...
            encoding_data_path = os.path.join('.', 'CMANGOES-2.0', 'Data_peptidereactor_full', 'Encodings', dataset,
                                              'CENACT_level_' + str(level) + '_' + alphabet_mode + '.csv')
            classes_path = os.path.join('.', 'CMANGOES-2.0', 'Data_peptidereactor_full', 'Original_datasets',
                                        dataset, 'classes.txt')
        ...
        pre_path = os.path.join('.', 'CMANGOES-2.0', 'Results_peptidereactor_full')
        if os.path.exists(pre_path) == False:
            os.mkdir(pre_path)

        results_path = os.path.join('.', 'CMANGOES-2.0', 'Results_peptidereactor_full', 'csv')
